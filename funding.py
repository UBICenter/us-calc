...
person['statefip'].replace({'1': 'Alabama', '2': 'Alaska', '4': 'Arizona', '5': 'Arkansas',
...
state_groups_taxinc = person.groupby(
    ['statefip'])[['weighted_state_tax', 'weighted_agi']].sum()
state_groups_taxinc.columns = ['state_tax_revenue', 'state_taxable_income']
person = person.merge(state_groups_taxinc, left_on=[
                      'statefip'], right_index=True)
...
text = dbc.Card([
    dbc.CardBody(
        [html.Div(id='ubi-output', style={
                "text-align": "left",
                "color": 'black',
                'fontSize': 25}),
         html.Div(id='winners-output', style={
             "text-align": "left",
             "color": 'black',
             'fontSize': 25}),
         html.Div(id='resources-output', style={
             "text-align": "left",
             "color": 'black',
             'fontSize': 25}),
...
app.layout = html.Div([
    ...
    dbc.Row(
        [
            dbc.Col(html.A([
                    html.Img(src="https://blog.ubicenter.org/_static/ubi_center_logo_wide_blue.png",
    ...
    dbc.Row(
        [
            dbc.Col(html.H1("Explore funding mechanisms of UBI", id="header",
                            style={'text-align': 'center', 'color': '#1976D2', 'fontSize': 50, 'letter-spacing': '4px', 'font-weight': 300}),
    ...
    dbc.Row(
        [
            dbc.Col(html.H1("The results of your reform:",
                            style={'text-align': 'center', 'color': '#1976D2', 'fontSize': 30}),
                    width={'size': 8, 'offset': 2},
                    ),
        ]),
    dbc.Row([
            dbc.Col(text, width={'size': 6, 'offset': 3})]),
    html.Br(),
    dbc.Row([
            dbc.Col(charts, width={'size': 10, 'offset': 1})
            ]),
    html.Br(),
    html.Br(),
    html.Br(),
...
def ubi(statefip, level, agi_tax, benefits, taxes, exclude):
    ...
    sub_spmu = target_spmu[['spmfamunit', 'year',
                            'new_resources', 'new_resources_per_person']]
    ...
    non_citizen_population = (
        target_persons.non_citizen * target_persons.asecwt).sum()
    non_citizen_child_population = (
        target_persons.non_citizen_child * target_persons.asecwt).sum()

    # Calculate total change in resources
    original_total_resources = (
        target_spmu.spmtotres * target_spmu.spmwt).sum()
    ...
    target_persons['original_poor'] = target_persons.spmtotres < target_persons.spmthresh

    # Calculate original poverty rate
    original_total_poor = (target_persons.original_poor *
                           target_persons.asecwt).sum()
    original_poverty_rate = (original_total_poor / population) * 100

    # Calculate the original poverty gap
    target_spmu['poverty_gap'] = np.where(target_spmu.spmtotres < target_spmu.spmthresh,
                                          target_spmu.spmthresh - target_spmu.spmtotres, 0)
    ...
    def pov_rate(column):
        return mdf.weighted_mean(target_persons[target_persons[column]], 'original_poor', 'asecwt') * 100
    ...
    target_persons['spm_resources_per_person'] = target_persons.spmtotres / \
    ...
    target_spmu['new_poverty_gap'] = np.where(target_spmu.new_resources < target_spmu.spmthresh,
                                              target_spmu.spmthresh - target_spmu.new_resources, 0)
    ...
    x2 = ['Child', 'Adult', 'People<br>with<br>disabilities',
    ...
    fig = go.Figure([go.Bar(x=x, y=[poverty_rate_change,
                            ...
                            text=[poverty_rate_change,
                                  poverty_gap_change,
                                  gini_change],
                            hovertemplate=['Original poverty rate: ' + original_poverty_rate_string + '%<br><extra></extra>'
                                           'New poverty rate: ' + poverty_rate_string + '%',
                                           'Original poverty gap: $' + original_poverty_gap_billions + 'B<br><extra></extra>'
                                           'New poverty gap: $' + poverty_gap_billions + 'B',
                                           'Original gini: <extra></extra>' + original_gini_string +
    ...
    fig.update_layout(uniformtext_minsize=10,
                      uniformtext_mode='hide', plot_bgcolor='white')
    ...
    if "state" in radio:
        return [
            ...
            {'label': '  Supplemental Security Income (SSI)',
             'value': 'incssi', "disabled": True},
            {'label': '  Snap (food stamps)',
             'value': 'spmsnap', "disabled": True},
            {'label': '  Earned Income Tax Credit',
             'value': 'eitcred', "disabled": True},
            {'label': '  Unemployment', 'value': 'incunemp', "disabled": True},
            {'label': '  Energy Subsidy (LIHEAP)',
             'value': 'spmheat', "disabled": True}
        ]
    else:
        return [
